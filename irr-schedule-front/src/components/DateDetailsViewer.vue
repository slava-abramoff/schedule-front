<template>
          ...
          <tbody>
            <tr v-for="(item, index) in tableData" :key="item.id" class="uk-text-nowrap">
              <td
                ...
                :key="key"
                @click="key !== 'actions' && startEditing(index, key)"
              >
                <!-- Для start_time - input type="time" -->
                <template v-if="key === 'start_time' && editingCell.row === index && editingCell.col === key">
                  <input
                ...
                <template v-else-if="key === 'end_time' && editingCell.row === index && editingCell.col === key">
                  <input
                    :value="toTimeInput(tableData[index][key])"
                    type="time"
                    class="uk-input uk-form-small"
                ...
                <template v-else-if="key === 'corps' && editingCell.row === index && editingCell.col === key">
                  <select
                ...
                <template v-else-if="key === 'actions'">
                  <button class="uk-button uk-button-danger uk-button-small" @click="deletedLecture(index)">
  ...
  import { getDay, updateLecture, createLecture, deleteLecture } from '../api/api';
  ...
  // Удаление лекции
  const deletedLecture = async (row) => {
    const lectureId = tableData.value[row].id;
